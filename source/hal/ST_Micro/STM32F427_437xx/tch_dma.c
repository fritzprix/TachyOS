...
static BOOL tch_dmaHandleIrq(tch_dma_handle_prototype* handle,tch_dma_descriptor* dma_desc);
...
__USER_RODATA__ tch_lld_dma DMA_IX = {
	...
	.allocate = tch_dmaOpenStream,
	.beginXfer = tch_dmaBeginXfer,
	.freeDma = tch_dmaClose
};
...
__USER_API__ static tch_DmaHandle tch_dmaOpenStream(const tch* env,dma_t dma,tch_DmaCfg* cfg,uint32_t timeout,tch_PwrOpt pcfg){
	...
	*dma_desc->_clkenr |= dma_desc->clkmsk;          // clk source is enabled
	...
	ins->destr = (tch_kobjDestr) tch_dmaClose;
	ins->env = env;
	ins->ch = cfg->Ch;
	ins->dma = dma;
	ins->status = 0;
