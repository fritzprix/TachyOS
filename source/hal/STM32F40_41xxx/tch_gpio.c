...
static tchStatus tch_gpio_handle_registerIoEvent(tch_gpio_handle* self,const tch_gpio_evCfg* cfg,const tch_IoEventCalback_t callback);
static tchStatus tch_gpio_handle_unregisterIoEvent(tch_gpio_handle* self);
static BOOL tch_gpio_handle_listen(tch_gpio_handle* self,uint32_t timeout,const tch_gpio_evCfg* cfg);
...
/**
 */
tchStatus tch_gpio_handle_registerIoEvent(tch_gpio_handle* self,const tch_gpio_evCfg* cfg,const tch_IoEventCalback_t callback){
	tch_gpio_handle_prototype* _handle = (tch_gpio_handle_prototype*) self;
...
tchStatus tch_gpio_handle_unregisterIoEvent(tch_gpio_handle* self){
	tch_gpio_handle_prototype* _handle = (tch_gpio_handle_prototype*) self;
...
BOOL tch_gpio_handle_listen(tch_gpio_handle* self,uint32_t timeout,const tch_gpio_evCfg* cfg){
	tch_gpio_handle_prototype* _handle = (tch_gpio_handle_prototype*) self;
	tch_ioInterrupt_descriptor* ioIrqObj = &IoInterrupt_HWs[_handle->pin];
	if(__get_IPSR())
		return FALSE;
...
void EXTI15_10_IRQHandler(void){
	...
	while(ext_pr){
		if(ext_pr & 1){
			ioIntObj = &IoInterrupt_HWs[10 + pos];
			if(!ioIntObj->io_occp)
				return;
