...
struct mem_nsegment {
	nrbtreeRoot_t         reg_root;           //  segment로부터 할당된 region들을 관리하기 위한 red black tree, region의 page offset을 key 값으로 같는다.
	nrbtreeNode_t         addr_rbn;           //  전역적으로 segment를 lookup 하기 위한 red black tree node로 segment의 page offset을 key 값으로 한다.
	nrbtreeNode_t         id_rbn;             //  addr_rbn과 같은 목적을 가지고 있다. 단, segment id값을 key값으로 갖는다.
...
struct mem_nregion {
	nrbtreeNode_t         reg_rbn;            //  segment의 reg_root에 추가하기 위한 red-black tree node 구조체
	nrbtreeNode_t         map_rbn;            //  thread control block에 추가하기 위한 red-black tree node 구조체
